# ClawTask × OpenClaw 完整集成方案

## 现状盘点

### CLI 已完成 ✅
ClawTask CLI 功能完整，可以通过 `npx tsx cli/index.ts` 操作所有任务：
...
### 未完成 ❌
1. **CLI 未打包发布** — 不能直接 `clawtask create`，要用 `npx tsx cli/index.ts`
2. **OpenClaw Skill 未创建** — agent 不知道 clawtask 存在
...
**目标**：让 `clawtask` 命令在终端全局可用。

```bash
# 用户安装
npm install -g clawtask
# 或开发时
pnpm link --global
```
...
# ~/.local/bin/clawtask
#!/bin/bash
CLAWTASK_DIR="/Users/cheche/workspace/clawtask"
exec npx --prefix "$CLAWTASK_DIR" tsx "$CLAWTASK_DIR/cli/index.ts" "$@"
```

### Phase B：创建 OpenClaw Skill

**目标**：OpenClaw agent 在对话中可以直接使用 clawtask 命令。

**文件**：`~/.openclaw/skills/clawtask/SKILL.md`（或发布到 clawhub）

```markdown
---
name: clawtask
description: >
  Manage tasks on ClawTask board. Create tasks, list pending tasks,
  ...
  openclaw:
    ...
    install:
      - id: npm
        ...
        package: clawtask
        bins: ["clawtask"]
        label: "Install ClawTask CLI (npm)"
---
...
```bash
clawtask list                          # 所有任务
clawtask list --status open            # 待处理
clawtask list --status running         # 执行中
clawtask show <number>                 # 任务详情
```

### 创建任务
```bash
clawtask create --title "任务标题" --description "详细描述"
clawtask create --title "写周报" --assign claude-code --schedule immediate
```

### 执行任务
```bash
clawtask start <number>                # 开始执行
clawtask log <number> "进展消息"       # 记录进展
clawtask complete <number> --summary "完成摘要"  # 完成
clawtask fail <number> --reason "原因"  # 标记失败
clawtask block <number> --reason "需要用户提供信息"  # 标记阻塞
```

### 管理 Agent
```bash
clawtask agent list                    # 列出 agent
clawtask agent checkin --name openclaw # 签到
```

## 规则

1. 不要主动创建任务，除非用户明确要求"记录"或"创建"
2. 大部分对话任务是即时完成的，不需要通过 ClawTask
3. 只有需要跟踪的、较长时间的任务才记录到 ClawTask
4. 创建任务时，title 要简洁明确，description 包含完整上下文
5. 执行过程中用 `clawtask log` 记录关键进展
6. 完成后用 `clawtask complete --summary` 写清楚结果
```
...
cp /path/to/SKILL.md ~/.openclaw/skills/clawtask/SKILL.md

# 方式 2：发布到 clawhub 后
clawhub install clawtask
```

### Phase C：Cron 巡检（Orchestrator 模式）

**目标**：OpenClaw agent 定期检查 ClawTask 中未分配的任务并自动认领执行。
...
openclaw cron add \
  --cron "*/10 * * * *" \
  --agent main \
  --message "请检查 ClawTask 任务列表：
运行 clawtask list --status open 查看待处理任务。
如果有未分配的任务，根据你的能力判断是否认领：
- 如果任务适合你，运行 clawtask assign <number> --agent openclaw 然后 clawtask start <number> 开始执行
- 执行过程中用 clawtask log <number> 记录进展
- 完成后用 clawtask complete <number> --summary '结果摘要'
- 如果遇到问题，用 clawtask block <number> --reason '原因' 通知用户"
```
...
每 10 分钟:
  OpenClaw agent 收到 cron 消息
    ↓
  运行 clawtask list --status open
    ↓
  如果有未分配任务 → 分析任务内容
    ↓
  适合的 → clawtask assign + start + 执行 + log + complete
...
async execute(context, onEvent) {
  ...
  // 在 message 中注入 clawtask CLI 指令，让 agent 自己记录日志：
  //   "执行过程中请用 clawtask log {taskNumber} 记录进展"
...
- clawtask log ${context.taskNumber} "你的进展消息"
- 完成后运行: clawtask complete ${context.taskNumber} --summary "结果摘要"
- 如果遇到问题: clawtask block ${context.taskNumber} --reason "问题描述"
`
}
```

这样 OpenClaw agent 在执行时会自己调用 clawtask CLI 写日志，Web 面板通过 SSE 实时显示。
...
| **A2** | 验证 `clawtask` 命令在终端可用 | 5 min |
| **B1** | 创建 SKILL.md | 10 min |
| **B2** | 复制到 ~/.openclaw/skills/clawtask/ | 2 min |
| **B3** | 验证 OpenClaw agent 可以使用 clawtask 命令 | 5 min |
...
OpenClaw agent (因为安装了 clawtask skill):
  → clawtask create --title "调研 AI agent 框架" --description "..." --assign openclaw --schedule immediate
  → "已创建任务 #5，开始执行"
