...
export async function executeTask(taskId: string, promptOverride?: string): Promise<void> {
  ...
  const onEvent = async (event: ExecutionEvent) => {
    ...
    if (event.type === 'blocked') {
      ...
      notify(buildTaskNotification('task.blocked', { id: taskId, number: task.number, title: task.title }, { blockReason: event.message }), { sourceChannel: task.sourceChannel, sourceAccountId: task.sourceAccountId, sourceTarget: task.sourceTarget }).catch(() => {})
      return
    }
  ...
  try {
    ...
    if (result.success) {
      ...
      if (maxRetries > 0 && retryCount <= maxRetries) {
        ...
        notify(buildTaskNotification('task.failed', { id: taskId, number: task.number, title: task.title }, { error: result.error }), { sourceChannel: task.sourceChannel, sourceAccountId: task.sourceAccountId, sourceTarget: task.sourceTarget }).catch(() => {})
      }
    }
